...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const squareX = (x1 - x2) * (x1 - x2);
  const squareY = (y1 - y2) * (y1 - y2);
...
function roundToPowerOfTen(num, pow) {
  const numberToRound = 10 ** pow;
  const roundedNum = Math.round(num / numberToRound);
  const result = roundedNum * numberToRound;
  return result;
}
